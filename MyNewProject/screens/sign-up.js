...
import { auth, database } from "F:/FYP-Swapit/fyp _project/MyNewProject/react-native-chat/config/firebase.js"; 
...
export default function CreateAccountPage({ navigation }) {
  ...
  const checkEmailExists = async (email) => {
    try {
      // Check MongoDB
      const mongoResponse = await axios.post('http://10.20.2.155:5000/check-email', { 
        email 
      });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await axios.post("http://10.20.2.155:5000/CreateAccount", userData, {
        timeout: 10000,
      });
