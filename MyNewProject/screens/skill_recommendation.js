...
// //       try {
// //         const tutorsResponse = await axios.get("http://10.20.3.112:5000/recommendedTutors");
...
//     const fetchRecommendations = async () => {
//       try {
//         const response = await axios.get("http://10.20.5.187:5000/recommendedTutors");
//         console.log("API Response:", response.data); // Debugging
  
//         if (response.data.status === "Ok" && response.data.data.length > 0) {
//           console.log("Setting Tutors:", response.data.data);
//           setRecommendedTutors(response.data.data);
...
export default function SkillRecommendationPage() {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        ...
        if (response.data.status === "Ok" && response.data.data.length > 0) {
          console.log("Setting Tutors:", response.data.data);
          setAllTutors(response.data.data); // Store all tutors
          setFilteredTutors(response.data.data.slice(0, 10)); // Show only the first 10 tutors initially
        } else {
          console.warn("No tutors found");
        }
      } catch (error) {
  ...
  const applyFilters = (search, Gender) => {
    let filtered = allTutors; // Use all tutors for filtering
    ...
    if (search.trim() !== "") {
      const normalizedQuery = search.trim().toLowerCase();
      filtered = filtered.filter(tutor => {
        const skills = tutor.skills_i_have ? tutor.skills_i_have.toLowerCase() : "";
        return skills.includes(normalizedQuery);
      });
    }

    // Update the filtered tutors to show only the first 10 results
    setFilteredTutors(filtered.slice(0, 10));
  };
