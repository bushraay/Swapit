...
function Chat({ route }) {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(doc(database, 'chats', route.params.id), async (document) => {
        const recipient = document.data().users.find(user => user.email !== auth?.currentUser?.email);
        const recipient_email = recipient.email; // Get recipient email
        
        setRecipientName(recipient_email); // Store recipient email instead of name
...
useFocusEffect(
  useCallback(() => {
      if (recipientName) {
          console.log("Setting Header Title to:", recipientName);
          navigation.setOptions({ title: recipientName }); // Force update
      }
