...
export default function HistoryPage() {
  ...
  const handleReviewSubmit = () => {
    console.log("Trade ID:", selectedTrade);
    console.log("Rating:", rating, "Comment:", comment);
    submitReview(selectedTrade.tradedWith, rating, comment);
    setPopupVisible(false);
    setRating(0);
    setComment("");
  };
//   const submitReview = async (reviewedUsername, rating, comment) => {
//     try {
//         const reviewData = { rating, comment };
//         const requestBody = { reviewedUsername, reviewData };

//         console.log("Sending request body:", requestBody); // Debugging log

//         const response = await fetch("http://10.20.5.187:5000/submit-review", {
//             method: "POST",
//             headers: {
//                 "Content-Type": "application/json",
//             },
//             body: JSON.stringify(requestBody), // Ensure JSON is correctly formatted
//         });
...
const submitReview = async (reviewedUsername, rating, comment) => {
  try {
      const reviewerUsername = auth?.currentUser?.displayName; // Ensure reviewer username is fetched correctly
      if (!reviewerUsername) {
          throw new Error("Reviewer username not found");
      }

      const reviewData = { reviewerUsername, rating, comment };
      const requestBody = { reviewedUsername, reviewData };
